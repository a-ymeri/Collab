    ...
    useEffect(() => {
        ...
        if (!stompClient.current.connected) {
            ...
            stompClient.current.connect({ auth: jwt }, function () {
                ...
                stompClient.current.subscribe('/topic/2', function (data: any) {
    ...
    function changeEditorValue(newValue: any) {
        {
            ...
            ops.forEach((op: any, index, ops) => { //(InsertTextOperation | RemoveTextOperation | InsertNodeOperation)) => {
                ...
                if (!op.siteID && editor && document.getElementById("thing") == document.activeElement) { //foreign operation
                    op.siteID = ID
                    op.stateID = stateID
                    stompClient.current.send('/app/sendcopy', {}, JSON.stringify(op))
                }
            });
        }
    }
